...
if __name__ == "__main__":
    ...
    logger.info(
        "Loading Test dataloader successfully! -- {}".format(
            time.time() - start_loading
        )
    )
    ...
    critetion = (
        build_losses(cfg.TRAIN.loss, LOSSES, default_args=dict(cfg=cfg.TRAIN.loss))
        .cuda()
        .to(torch.float64)
    )
    if cfg.TRAIN.optimizer == "Adam":
        ...
        optimizer = SAM(
            model.parameters(), optim.Adam, lr=cfg.TRAIN.lr, weight_decay=1e-4
        )
    else:
        optimizer = optim.SGD(
            model.parameters(), lr=cfg.TRAIN.lr, weight_decay=1e-5, momentum=0.9
        )
    ...
    if len(cfg.TRAIN.gpus) > 0:
        model = (
            nn.DataParallel(model, device_ids=cfg.TRAIN.gpus).cuda().to(torch.float64)
        )
    else:
    ...
    if (
        isinstance(cfg.TRAIN.lr_scheduler, dict)
        and cfg.TRAIN.lr_scheduler.type == "MultiStepLR"
    ):
        scheduler_type = cfg.TRAIN.lr_scheduler.type
        scheduler_args = dict(cfg.TRAIN.lr_scheduler)
        scheduler_args.pop("type", None)
        ...
        lr_scheduler = LinearDecayLR(
            ...
            cfg.TRAIN.epochs,
            cfg.TRAIN.epochs // 4,
            last_epoch=cfg.TRAIN.get("begin_epoch", 0),
            booster=4,
        )
